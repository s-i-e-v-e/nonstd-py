...
def list_all(path: str):
    return List(os.listdir(path)).map(lambda x: File(x, path, os.path.join(path, x)))


def list_files(path: str):
    return list_all(path).filter(lambda x: os.path.isfile(x.full_path))


def list_dirs(path: str):
    return list_all(path).filter(lambda x: os.path.isdir(x.full_path))
